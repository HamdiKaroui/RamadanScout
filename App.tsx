...
const App: React.FC = () => {
  ...
  const stats = useMemo<AppStats>(() => {
    ...
    // Each person gets 5 mins per delivery point assigned to them
  ...
  const processWorkbook = (workbook: XLSX.WorkBook): DeliveryPoint[] => {
    ...
    return data.slice(1).map((row: any[]) => {
      ...
      // Ensure we have at least a code or a name to create a point
  ...
  const processCommand = async () => {
    ...
    try {
      ...
      const estimatedTime = ordered.length * 5; // 5 mins per person
          ...
          {Object.keys(state.assignments).length > 0 && (
            <div className="space-y-6">
              ...
              {(Object.entries(state.assignments) as [string, OptimizedRoute][]).map(([name, route]) => (
                <div key={name} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
